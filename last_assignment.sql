...
FROM sakila.customer AS c
LEFT JOIN paying_customers AS pc
  ON c.customer_id = pc.customer_id
...
CROSS JOIN avg_rental_rate AS a
...
SELECT c.customer_id, c.first_name, c.last_name
FROM sakila.customer AS c
JOIN customers_in_same_month AS cm
...
CREATE TEMPORARY TABLE tmp_avg_payment AS
SELECT AVG(amount) AS avg_amount
FROM sakila.payment;

CREATE TEMPORARY TABLE tmp_staff_gt_avg AS
SELECT DISTINCT p.staff_id
FROM sakila.payment AS p
CROSS JOIN tmp_avg_payment AS a
WHERE p.amount > a.avg_amount;

SELECT s.*
FROM sakila.staff AS s
JOIN tmp_staff_gt_avg AS t
...
CREATE VIEW vw_film_categories AS
SELECT
    f.film_id,
    f.title,
    c.category_id,
...
-- SELECT * FROM vw_film_categories;
...
CREATE VIEW vw_customer_staff_emails AS
SELECT
...
-- SELECT * FROM vw_customer_staff_emails;



-- 15. Find all actors who acted in the film "ACADEMY DINOSAUR".

CREATE VIEW vw_actors_academy_dinosaur AS
SELECT
    a.actor_id,
    a.first_name,
    a.last_name
...
SELECT *
FROM vw_customer_rental_counts
